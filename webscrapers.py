...
class Sauce:
    ...
    def getSauceHtml(self,id:int,verb: bool): # get the HTML of a given id
        link = 'https://nhentai.net/g/'+str(id)+'/'
        print(link)
        return self.getHtml(link,verb)
    def getHtml(self,link:str,verb: bool): #get HTML of given page
        raw = reqs.get(link)
...
class masterIndices: # the master index for all of the tags for ease of serialization (EX: an artist has is converted to a number)
    ...
    def getHtml(self,link:str,verb: bool): #get HTML of given page
        ...
        if raw.status_code == 200:
            ...
            if verb:
                print('raw retieval failed')
    def getPageCnt(self,path): #get page count from one of the tag pages
